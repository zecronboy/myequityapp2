...
# --- UI STYLING MASTER TEMPLATE (Typography Upgrades) ---
st.markdown("""
    <style>
        /* Sidebar Zoom Adjustment */
        div[data-testid="stSidebar"] [data-testid="stDataFrame"] {
            font-size: 15px !important;
            zoom: 1.1;
        }

        /* Tighter Custom Table + Increased Font Sizes! */
        ...
        .custom-table td { font-size: 16px; font-weight: 500; border-bottom: 1px solid #e0e0e0; padding: 14px 10px; line-height: 1.6; vertical-align: middle;} /* 16px Main metric text! */
        ...
        .earn-date { font-family: monospace; color: #2c3e50; font-size: 15px; font-weight:600;}
        .earn-past { font-family: monospace; color: #e74c3c; font-size: 12px; font-weight: 600; background: #fadbd8; padding: 3px 6px; border-radius: 4px; margin-left: 5px; vertical-align: top;}
        
        /* Shrunken Meta headers below big financial metrics */
        .meta-lbl { font-size:12px; color: #95a5a6; font-weight:700; text-transform:uppercase; letter-spacing: 0.3px;}
    </style>
""", unsafe_allow_html=True)

# Trillion / Billion formatter
    ...
    with st.spinner("Executing broad API pulls & formatting UI matrices..."):
        ...
        for idx, row in enumerate(valid_rows):
            ...
            try:
                ...
                pe_combined = f"<span class='meta-lbl'>TTM:</span> <strong>{f'{pe_ttm:.1f}' if isinstance(pe_ttm, float) else '-'}</strong><br>" \
                              f"<span class='meta-lbl'>FWD:</span> <strong>{f'{pe_fwd:.1f}' if isinstance(pe_fwd, float) else '-'}</strong>"
                ...
                rev_comp = f"<span class='meta-lbl'>Latest (Q): </span><strong>{rev_str}</strong> <span style='font-size:11px;color:#c0392b;font-weight:700;' title='Institutional % surprise poll expectations paywalled.'>(Surprise% Paywall)</span><br>" 
                rev_comp += f"<span class='meta-lbl'>Y/Y Gro:</span> <strong>{f'{rev_yoy:.1f}%' if rev_yoy else '-'}</strong> <span style='color:grey'>|</span> " \
                ...
                if earn_ts:
                    ...
                    earn_date_str = f"<span class='earn-date'>{earn_dt.strftime('%b %d')}</span>{status_lbl}"
                ...
                data_outputs.append({"Category": cat, "Ticker": t, "Name": co_name, "MktCap": mcap, "PriceBox": price_f, "PEBox": pe_combined, "EPSBox": eps_comp, "RevBox": rev_comp, "Earnings": earn_date_str, "Sentiment": drama_alert})
                
            except Exception as e:
                data_outputs.append({ "Category": cat, "Ticker": t, "Name": "Fail", "MktCap": "ERR", "PriceBox": "Err", "PEBox": "-", "EPSBox": "-", "RevBox": "-", "Earnings": "-", "Sentiment": "-"})
...
if 'master_df' in st.session_state:
    ...
    for category in unique_cats:
        ...
        # Hardcoding the breadths correctly across the table so columns act right!
        table_html = "<table class='custom-table'>"
        table_html += "<tr><th style='width: 8%'>Asset</th><th style='width: 9%'>Mkt Cap</th><th style='width: 14%'>Daily Tape</th><th style='width: 8%'>P/E Ratios</th><th style='width: 17%'>Quarterly EPS vs Street</th><th style='width: 19%'>Forward Rev Strength</th><th style='width: 12%'>Catalyst Log</th><th>AI Sector Scrub</th></tr>"
        
        for _, r in cat_df.iterrows():
            table_html += f"<tr>"
            # Title Injection causes full name rendering hovering
            table_html += f"<td class='ticker-col' title='Corporate Title: {r['Name']}'>{r['Ticker']}</td>" 
